...
def EBGAN2(input_shape, batch_size=128, load_weights=False, num_layers_g=4, num_layers_d=4, noise=None, train_disc=True,
    ...
    if train_disc:
        l3 = sub(d_y, y_input)
        gan = Model(input=[x_input, y_input], output=[l1, l2, l3])
        gan.compile(loss=[l2_loss] + [mse] * 2, loss_weights=[-d_weight, -1.0, r_weight], optimizer=optimizer)
        ...
        gan.compile(loss=[l2_loss, mse, l2_loss], loss_weights=[d_weight, 1.0, d_weight], optimizer=optimizer)
