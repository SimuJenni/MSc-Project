...
def l2_mb(y_true, y_pred):  # Idea: could pass in margin during training (similar to noise thingie)
    ...
    return K.mean(K.minimum(K.square(y_pred), 1.0), axis=-1)
...
def EBGAN2(input_shape, batch_size=128, load_weights=False, num_layers_g=4, num_layers_d=4, noise=None, train_disc=True,
    ...
    if train_disc:
        l3 = sub(d_y, y_input)
        gan = Model(input=[x_input, y_input], output=[l1, l2, l3])
        gan.compile(loss=[l2_ms, l2_ms, l2_loss], loss_weights=[-1.0, -d_weight, d_weight], optimizer=optimizer)
        ...
        gan.compile(loss=[l2_ms, l2_ms, l2_mb], loss_weights=[1.0, d_weight, r_weight], optimizer=optimizer)
