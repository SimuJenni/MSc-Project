...
def l2_mb(y_true, y_pred):
    return K.mean(K.maximum(K.abs(y_pred), 10.0), axis=-1)


def l2_ms(y_true, y_pred):
    return K.mean(K.maximum(K.square(y_pred), 0.2), axis=-1)
...
def EBGAN2(input_shape, batch_size=128, load_weights=False, num_layers_g=4, num_layers_d=4, noise=None, train_disc=True,
    ...
    l1 = sub(de_g_x, de_y)
    # l1 = sub(d_g_x, d_y)  # 19870
    l2 = sub(d_g_x, g_x)
    if train_disc:
        l3 = sub(d_y, y_input)
        gan = Model(input=[x_input, y_input], output=[l1, l2, l3])
        gan.compile(loss=[mae, l2_ms, l2_loss], loss_weights=[-1.0, -d_weight, d_weight], optimizer=optimizer)
        ...
        gan.compile(loss=[mae, l2_ms, l2_loss], loss_weights=[1.0, d_weight, r_weight], optimizer=optimizer)
