...
class ToonNetTester:
    ...
    def test_reconstruction(self):
        ...
        with self.sess.as_default():
            with self.graph.as_default():
                imgs_test, toons_test = self.get_toon_test_batch()
                labels_disc = self.model.disc_labels()

                # Create the model
                dec_im, dec_gen, dec_spatial_drop, dec_eature_drop, dec_shuffle, disc_out = \
                ...
                summary_ops = [tf.image_summary('images/generator', montage_tf(dec_gen[:24], 4, 6), max_images=1),
                               tf.image_summary('images/autoencoder', montage_tf(dec_im[:24], 4, 6), max_images=1),
                               tf.image_summary('images/dec_spatial_drop', montage_tf(dec_spatial_drop[:24], 4, 6), max_images=1),
                               tf.image_summary('images/dec_eature_drop', montage_tf(dec_eature_drop[:24], 4, 6), max_images=1),
                               tf.image_summary('images/dec_shuffle', montage_tf(dec_shuffle[:24], 4, 6), max_images=1),
