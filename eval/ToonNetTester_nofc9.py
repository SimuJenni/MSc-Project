    ...
    def test_reconstruction(self):
        ...
        with self.sess.as_default():
            with self.graph.as_default():
                ...
                summary_ops = [tf.image_summary('images/autoencoder', montage_tf(dec_im[:24], 4, 6), max_images=1),
                               tf.image_summary('images/dec_spatial_drop', montage_tf(dec_pdrop[:24], 4, 6), max_images=1),
                               tf.image_summary('images/dec_eature_drop', montage_tf(dec_cdrop[:24], 4, 6), max_images=1),
                               tf.image_summary('images/dec_shuffle1', montage_tf(dec_shuffle1[:24], 4, 6), max_images=1),
                               tf.image_summary('images/dec_shuffle2', montage_tf(dec_shuffle2[:24], 4, 6), max_images=1),
