        ...
        with tf.variable_scope('discriminator', reuse=reuse):
            with slim.arg_scope(toon_net_argscope(activation=tf.nn.relu, padding='SAME', training=training,
                ...
                if with_fc:
                    ...
                    net = slim.flatten(net)
                    net = slim.fully_connected(net, 4096, scope='fc1', trainable=with_fc, activation_fn=lrelu)
                    net = slim.dropout(net, 0.5, is_training=training)
                    net = slim.fully_connected(net, 4096, scope='fc2', trainable=with_fc, activation_fn=lrelu)
                    net = slim.dropout(net, 0.5, is_training=training)
