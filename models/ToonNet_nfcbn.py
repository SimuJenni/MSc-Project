    ...
    def discriminate(self, net, reuse=None, training=True, with_fc=True, pad='VALID'):
        ...
        with tf.variable_scope('discriminator', reuse=reuse):
            with slim.arg_scope(alexnet_argscope(activation=self.fc_activation, padding='SAME', training=training)):
                ...
                if with_fc:
                    net = slim.conv2d(net, 1, kernel_size=[3, 3], stride=1, activation_fn=tf.nn.tanh, scope='conv_6',
                                      padding='VALID', normalizer_fn=None, biases_initializer=tf.zeros_initializer)
