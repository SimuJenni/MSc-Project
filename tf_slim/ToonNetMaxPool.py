...
class AEGAN4:
    ...
    def net(self, img, cartoon, edges, reuse=None, training=True):
        ...
        if training:
            disc_in += tf.random_normal(shape=tf.shape(disc_in),
                                        stddev=0.05+noise_amount(0.333*self.num_ep*self.data_size/self.batch_size))
    ...
    def gen_labels(self):
        """Generates labels for generator training (see discriminator input!).
