...
class VAEGAN:
    ...
    def build_classifier(self, img, num_classes, reuse=None, training=True):
        ...
        """
        model, _ = discriminator(img, reuse=reuse, num_out=num_classes, training=training, train_fc=False)
        with tf.variable_scope('fully_connected', reuse=reuse):
            with slim.arg_scope(toon_net_argscope(training=training)):
                model = slim.fully_connected(model, num_classes, scope='fc3',
                                           activation_fn=None,
                                           normalizer_fn=None,
...
def discriminator(net, reuse=None, num_out=2, training=True, train_fc=True):
    ...
    with tf.variable_scope('discriminator', reuse=reuse):
        with slim.arg_scope(toon_net_argscope(activation=lrelu, padding='SAME', training=training)):
            ...
            net = slim.fully_connected(net, 4096, scope='fc1', trainable=train_fc, activation_fn=tf.nn.relu)
            net = slim.dropout(net, 0.5, is_training=training)
            net = slim.fully_connected(net, 4096, scope='fc2', trainable=train_fc, activation_fn=tf.nn.relu)
            net = slim.dropout(net, 0.5, is_training=training)
