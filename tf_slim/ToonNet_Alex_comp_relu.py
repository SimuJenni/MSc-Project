...
class VAEGAN:
    def __init__(self, num_layers, batch_size):
        ...
        """
        self.name = 'AEGAN_Alex_comp_relu_0001'
...
def discriminator(net, reuse=None, num_out=2, training=True, train_fc=True, w_reg=0.00001):
    ...
    with tf.variable_scope('discriminator', reuse=reuse):
        with slim.arg_scope(toon_net_argscope(activation=lrelu, padding='SAME', training=training, w_reg=w_reg)):
...
def classifier(net, num_classes, reuse=None, training=True, activation=tf.nn.relu, w_reg=0.0001):
    ...
    with tf.variable_scope('fully_connected', reuse=reuse):
        with slim.arg_scope(toon_net_argscope(activation=activation, training=training, w_reg=w_reg)):
...
def toon_net_argscope(activation=tf.nn.relu, kernel_size=(3, 3), padding='SAME', training=True, w_reg=0.00001):
