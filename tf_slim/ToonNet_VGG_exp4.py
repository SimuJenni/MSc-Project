...
DEFAULT_FILTER_DIMS = [64, 128, 256, 256, 256]
REPEATS = [1, 1, 2, 2, 2]
...
class VAEGAN:
    ...
    def experiment_net1(self, img, cartoon, edges, reuse=None, training=True):
        """Builds the AEGAN with the given inputs.
        ...
        disc_out, _, _ = discriminator(disc_in, num_layers=5, reuse=reuse, num_out=2, training=training)
        return img, dec_gen, disc_out

    def net_exp3(self, img, cartoon, edges, reuse=None, training=True):
        """Builds the AEGAN with the given inputs.
    ...
    def classifier(self, img, edge, num_classes, type='generator', reuse=None, training=True, fine_tune=True):
        ...
        if not fine_tune:
            ...
            _, _, _, model = generator(gen_in, num_layers=5, reuse=reuse, training=training)
