...
from tf_slim.layers import lrelu, up_conv2d, add_noise_plane, merge, spatial_dropout, bottleneck_res_layer
...
def generator(inputs, num_layers=5, reuse=None, training=True):
    ...
    Args:
        inputs: Input to the generator (i.e. cartooned image and/or edge-map)
    ...
    with tf.variable_scope('generator', reuse=reuse):
        with slim.arg_scope(toon_net_argscope(padding='SAME', training=training)):
            ...
            for r in range(NUM_RES):
                net = bottleneck_res_layer(net, f_dims[num_layers-1], 64, NOISE_CHANNELS[num_layers-1],
                                           scope='res_{}'.format(r+1))
...
def encoder(inputs, num_layers=5, reuse=None, training=True):
    ...
    Args:
        inputs: Input to the encoder (image)
        num_layers: Number of convolutional down-sampling layers
        reuse: Whether to reuse already defined variables
        training: Whether in train or test mode.
...
def discriminator(inputs, num_layers=5, reuse=None, num_out=2, training=True):
    ...
    Args:
        inputs: Input to the discriminator
        num_layers: Number of convolutional down-sampling layers
        reuse: Whether to reuse already defined variables
