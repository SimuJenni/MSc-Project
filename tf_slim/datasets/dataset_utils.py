...
def cartooned_example(image_data, toon_data, edge_data, image_format, height, width, class_id):
    return tf.train.Example(features=tf.train.Features(feature={
        'image/encoded': bytes_feature(image_data),
        'image/format': bytes_feature(image_format),
        'class/label': int64_feature(class_id),
        'image/height': int64_feature(height),
        'image/width': int64_feature(width),
        'edges/encoded': bytes_feature(edge_data),
        'edges/format': bytes_feature(image_format),
        'cartoon/encoded': bytes_feature(toon_data),
        'cartoon/format': bytes_feature(image_format),
    }))


def imagenet_example(filename, image_buffer, edges, cartoon, label, synset, human,
                     height, width):
    colorspace = 'RGB'
    channels = 3
    image_format = 'JPEG'
    return tf.train.Example(features=tf.train.Features(feature={
        ...
        'edges/encoded': bytes_feature(edges),
        'edges/format': bytes_feature(image_format),
        'cartoon/encoded': bytes_feature(cartoon),
        'cartoon/format': bytes_feature(image_format),
    })
    )
