...
with sess.as_default():
    with g.as_default():
        ...
        with tf.device('/cpu:0'):
            ...
            imgs_test, labels_test = tf.train.batch([img_test, label_test], batch_size=BATCH_SIZE, num_threads=8)
        ...
        learning_rate = tf.select(tf.python.math_ops.greater(global_step, num_train_steps / 2),
                                  0.01 - 0.01 * (2*tf.cast(global_step, tf.float32)/num_train_steps-1.0), 0.01)
