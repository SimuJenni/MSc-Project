...
class DCGAN(object):
    ...
    def build_model(self, is_train):
        ...
        self.g_conv_vars = [var for var in t_vars if 'g_s' in var.name]
        # print("var.name", var.name)
        # print("self.g_conv_vars", self.g_conv_vars)
        # print("self.g_vars", self.g_vars)
    ...
    def train(self, config, run_string="???"):
        ...
        d_optim = tf.train.AdamOptimizer(config.learning_rate, beta1=config.beta1) \
                          .minimize(5*self.d_loss+1*self.d_loss_feat, var_list=self.d_vars, global_step=global_step)
        ...
        g_optim = tf.train.AdamOptimizer(learning_rate=config.learning_rate, beta1=config.beta1) \
                          .minimize(5*self.g_loss+0.5*self.c_loss+1*self.l2_loss+1*self.g_loss_feat, var_list=self.g_vars)
    ...
    def discriminator_feature(self, image, sketches, reuse=False, y=None):
        ...
        # concated = tf.concat(3, [image, sketches])
        self.d_h0 = lrelu(conv2d(image, self.df_dim, name='d_sh0_conv'))
        ...
        h1 = linear(tf.reshape(self.d_h0, [self.batch_size, -1]), 1, 'd_sh1_lin')
    ...
    def generator(self, sketches_or_abstract_representations, z=None, y=None, from_abstract_representation=False,gflag=False,pred=False):
        ...
        if gflag:
            ...
            h1 = deconv2d(used_abstract, [self.batch_size, self.abstract_size * 2,
                                                         self.abstract_size * 2, self.gf_dim*2 + self.z_dim],name='g_h1')
