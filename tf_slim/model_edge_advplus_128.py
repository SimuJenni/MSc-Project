...
class DCGAN(object):
    def __init__(self, sess,
                 batch_size=256, sample_size = 128, image_shape=[256, 256, 3],
    ...
    def build_model(self, is_train):
        ...
        sketches, images = get_chair_pipeline_training_from_dump('edge_256x256/edge_image_imagenet_256x256.tfrecords',
                                                                 self.batch_size,
                                                                 55, image_size=self.image_size,resize_size=self.sample_size,
        ...
        with tf.variable_scope('generator') as scope:
            self.G = self.generator(self.sketches, self.z,gflag=True)
            # print("G", self.G)
            if not is_train:
                self.G_abstract = self.generator(self.abstract, from_abstract_representation=True,gflag=True)
            scope.reuse_variables()
            self.test_G = self.generator(self.test_sketches, self.z,gflag=True)
        ...
        with tf.variable_scope('comparator_loss') as scope:            
            self.c_loss = tf.reduce_mean(tf.square((self.D) - (self.D_)))
            self.ctest_loss = tf.reduce_mean(tf.square((self.testD) - (self.testD_)))

        with tf.variable_scope('L2') as scope:
            # whitened_generated = normalize_batch_of_images(self.G)
            # whitened_gt = normalize_batch_of_images(self.images)
        ...
        self.saver = tf.train.Saver(self.d_vars + self.g_vars + 
                                    ...
                                    max_to_keep=0)
        # self.saver = tf.train.Saver(self.l2_vars + self.c_vars+
        #                             batch_norm.shadow_variables,
        #                             max_to_keep=0)
    ...
    def train(self, config, run_string="???"):
        ...
        d_optim = tf.train.AdamOptimizer(config.learning_rate, beta1=config.beta1) \
                          .minimize(self.d_loss - self.c_loss, var_list=self.d_vars, global_step=global_step)
        ...
        g_optim = tf.train.AdamOptimizer(learning_rate=self.g_lr, beta1=config.beta1) \
                          .minimize(self.g_loss+self.l2_loss+self.c_loss, var_list=self.g_vars)
        ...
        try:
            ...
            while not coord.should_stop():
                ...
                _, _, errD_fake, errD_real, errG, errL2,errC = self.sess.run([d_optim, g_optim, self.d_loss_fake,
                                                                  self.d_loss_real, self.g_loss, self.l2_loss,self.c_loss])
                # _, _, errC0,errC1,errC2,errC3, errL2 = self.sess.run([l2_optim,c_optim, self.c0_loss,
                #                                             self.c1_loss,self.c2_loss,self.c3_loss, self.l2_loss])
                ...
                # hh1,hh2,hh3 = self.sess.run([self.hh1,self.hh2,self.hh3])
                # _,  errL2 = self.sess.run([l2_optim, self.l2_loss])
                ...
                print("Run: %s, Step: [%4d] time: %5.1f, iter: %1.2f, c_loss: %.8f,d_loss_F: %.8f,d_loss_R: %.8f,g_loss: %.8f, L2_loss: %.8f,add_G:%d"
                    % (run_string, counter, toc - start_time, duration, errC,errD_fake,errD_real,errG,errL2, additional_G_runs))
                ...
                if np.mod(counter, 500) == 2:
                    samples, sample_images, sample_sketches,test_samples,test_images,test_sketches = self.sess.run([self.G, self.images, self.sketches,self.test_G, self.test_images, self.test_sketches])
                    ...
                    # save_images(test_sketches, grid, os.path.join(config.summary_dir, '%s_test_edge.png' % counter))
    ...
    def discriminator(self, image, sketches=None, reuse=False, y=None):
        if reuse:
    ...
    def generator(self, sketches_or_abstract_representations, z=None, y=None, from_abstract_representation=False,gflag=False):
