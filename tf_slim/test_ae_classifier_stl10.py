...
if finetuned:
    MODEL_PATH = os.path.join(LOG_DIR, '{}_{}_finetune_{}_Retrain{}_final_ae/'.format(
        data.NAME, model.name, net_type, NUM_CONV_TRAIN))
    LOG_PATH = os.path.join(LOG_DIR, '{}_{}_finetune_{}_Retrain{}_final_ae/'.format(
        data.NAME, model.name, net_type, NUM_CONV_TRAIN))
else:
...
with sess.as_default():
    with g.as_default():
        ...
        # Get predictions
        preds_test = model.ae_classifier(imgs_test, data.NUM_CLASSES, training=False)
