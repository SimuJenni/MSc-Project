...
if finetuned:
    MODEL_PATH = os.path.join(LOG_DIR, '{}_{}_finetune_{}_Retrain{}_andanothersetting_0.01weight/'.format(
        data.NAME, model.name, net_type, NUM_CONV_TRAIN))
    LOG_PATH = os.path.join(LOG_DIR, '{}_{}_finetune_{}_Retrain{}_eval_andanothersetting_0.01weight/'.format(
        data.NAME, model.name, net_type, NUM_CONV_TRAIN))
else:
