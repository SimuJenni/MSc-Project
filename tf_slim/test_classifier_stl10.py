...
if finetuned:
    MODEL_PATH = os.path.join(LOG_DIR, '{}_{}_finetune_{}_Retrain{}_final_1weight/'.format(
        data.NAME, model.name, net_type, NUM_CONV_TRAIN))
    LOG_PATH = os.path.join(LOG_DIR, '{}_{}_finetune_{}_Retrain{}_final_eval_1weight/'.format(
        data.NAME, model.name, net_type, NUM_CONV_TRAIN))
else:
