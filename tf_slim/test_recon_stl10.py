...
with sess.as_default():
    with g.as_default():
        ...
        summary_ops.append(tf.summary.image('images/generator', montage_tf(gen_rec, 14, 14), max_images=1))
        summary_ops.append(tf.summary.image('images/ae', montage_tf(img_rec, 14, 14), max_images=1))
        summary_ops.append(tf.summary.image('images/ground-truth', montage_tf(imgs_test, 14, 14), max_images=1))
        summary_ops.append(tf.summary.image('images/cartoons', montage_tf(toons_test, 14, 14), max_images=1))
        summary_ops.append(tf.summary.image('images/edges', montage_tf(edges_test, 14, 14), max_images=1))
        with tf.variable_scope('discriminator', reuse=True):
            weights_disc_1 = slim.variable('conv_1/conv_1_1/weights')
        summary_ops.append(tf.summary.image('images/weights_disc_1', weights_montage(weights_disc_1, 8, 8),
                                            max_images=1))
