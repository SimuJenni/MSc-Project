...
SAVE_DIR = os.path.join(LOG_DIR, '{}_{}_newObjGen/'.format(data.NAME, model.name))
TEST = False
NUM_IMG_SUMMARY = 8
...
with sess.as_default():
    with g.as_default():
        ...
        dL_gen = slim.losses.sigmoid_cross_entropy(disc_out, 0.5*tf.ones_like(disc_out), scope=gen_loss_scope, weight=1.0)
