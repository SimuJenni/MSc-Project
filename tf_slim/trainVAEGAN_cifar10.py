...
TEST_SET_NAME = 'test'
model = VAEGAN(num_layers=4, batch_size=512, data_size=data.SPLITS_TO_SIZES[TRAIN_SET_NAME], num_epochs=300)
TARGET_SHAPE = [32, 32, 3]
LR = 0.0001
RESIZE_SIZE = max(TARGET_SHAPE[0], data.MIN_SIZE)
SAVE_DIR = os.path.join(LOG_DIR, '{}_{}_andanothersetting_100weight/'.format(data.NAME, model.name))
TEST = False
NUM_IMG_SUMMARY = 8
...
with sess.as_default():
    with g.as_default():
        ...
        l2_gen = slim.losses.sum_of_squares(gen_rec, imgs_train, scope=gen_loss_scope, weight=100.0)  #Before 30
        l2_mu = slim.losses.sum_of_squares(gen_mu, enc_mu, scope=gen_loss_scope, weight=10.0)   #Before 3
        l2_sigma = slim.losses.sum_of_squares(gen_logvar, enc_logvar, scope=gen_loss_scope, weight=1.0)  #Before 3
