...
LR = 0.0002
SAVE_DIR = os.path.join(LOG_DIR, '{}_{}_exp1/'.format(data.NAME, model.name))
TEST = False
NUM_IMG_SUMMARY = 6
...
with sess.as_default():
    with g.as_default():
        ...
        img_rec, gen_rec, disc_out = \
            model.experiment_net1(imgs_train, toons_train, edges_train)
        ...
        if TEST:
            img_rec_test, gen_rec_test, _, _, _ = model.net(imgs_test, toons_test, edges_test, reuse=True,
                                                            training=False)
            tf.summary.image('images/generator', montage_tf(gen_rec_test, NUM_IMG_SUMMARY, NUM_IMG_SUMMARY),
                             max_images=1)
            tf.summary.image('images/ae', montage_tf(img_rec_test, NUM_IMG_SUMMARY, NUM_IMG_SUMMARY), max_images=1)
            tf.summary.image('images/ground-truth', montage_tf(imgs_test, NUM_IMG_SUMMARY, NUM_IMG_SUMMARY),
                             max_images=1)
