...
class ToonNetTrainer:
    ...
    def generator_loss(self, disc_out, labels_gen, imgs_gen, imgs_train):
    ...
    def train(self, chpt_path=None):
        ...
        with self.sess.as_default():
            with self.graph.as_default():
                ...
                # Create the model
                img_rec, img_gen, disc_out = self.model.net(imgs_train, toons_train)

                # Compute losses
                disc_loss = self.discriminator_loss(disc_out, labels_disc)
                ae_loss = self.autoencoder_loss(img_rec, imgs_train)
                gen_loss = self.generator_loss(disc_out, labels_gen, img_gen, imgs_train)
